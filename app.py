    ...
    def on_close(self, ws: websocket.WebSocketApp, close_status_code: int, close_msg: str):
        ...
        with self._reconnect_lock:
            if not self.trading_system.is_stopping and self._connection_attempts < self._max_reconnect_attempts:
                ...
                backoff_time = min(5 * (2 ** (self._connection_attempts - 1)), 60)
...
class BacktestEngine:
            ...
            for i, row in df.iterrows():
                ...
                if row['buy_signal'] and position_size <= 0:
                    if position_size < 0:
                        ...
                        capital += pnl
                        trades.append({'type': 'close_short', 'price': current_price, 'timestamp': timestamp, 'pnl': pnl, 'capital_after_trade': capital})
                        position_size = 0
                    if capital > 0:
                        units_to_buy = capital / current_price
                        position_size += units_to_buy
                        trades.append({'type': 'buy', 'entry_price': current_price, 'timestamp': timestamp, 'position_size': position_size, 'capital_after_trade': capital})
                    ...
                    if position_size > 0:
                        ...
                        capital += pnl
                        trades.append({'type': 'close_long', 'price': current_price, 'timestamp': timestamp, 'pnl': pnl, 'capital_after_trade': capital})
                        position_size = 0
                    if capital > 0:
                        units_to_sell = capital / current_price
                        position_size -= units_to_sell
                        trades.append({'type': 'sell', 'entry_price': current_price, 'timestamp': timestamp, 'position_size': position_size, 'capital_after_trade': capital})
    ...
    def calculate_performance_metrics(self, equity_curve: List[Dict], trades: List[Dict], 
        ...
        return {
            'strategy_name': strategy_name, 'symbol': symbol, 'start_date': start_date, 'end_date': end_date,
            'initial_capital': initial_capital, 'final_equity': final_equity, 'total_return': total_return,
            'sharpe_ratio': sharpe_ratio, 'max_drawdown': max_drawdown, 'total_trades': total_trades,
    ...
    def save_backtest_results(self, results: Dict):
        ...
        with self.db_manager.get_connection() as conn:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                results.get('strategy_name', ''), results.get('symbol', ''), results.get('start_date', ''),
                results.get('end_date', ''), results.get('total_return', 0.0), results.get('sharpe_ratio', 0.0),
                results.get('max_drawdown', 0.0), results.get('total_trades', 0), results.get('win_rate', 0.0),
...
class TradingSystem:
        ...
        if order_id:
            ...
            with self.db_manager.get_connection() as conn:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    order_id, data.get('product_id'), data.get('symbol'), data.get('size'), data.get('side'),
                    data.get('order_type', 'market_order'), str(data.get('limit_price')) if data.get('limit_price') is not None else None,
                    str(data.get('stop_price')) if data.get('stop_price') is not None else None, data.get('state'),
                    data.get('client_order_id'), data.get('created_at'), data.get('unfilled_size'),
    ...
    def process_market_data(self, data: MarketData):
        ...
        with self.db_manager.get_connection() as conn:
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (data.symbol, data.price, data.timestamp, data.volume, data.bid, data.ask))
            conn.commit()
    ...
    def get_system_status(self) -> Dict:
        return {
            "is_running": self.is_running, "websocket_connected": self.ws_manager.is_connected,
            "websocket_authenticated": self.ws_manager.is_authenticated, "subscribed_channels": list(self.ws_manager.subscribed_channels),
            "active_orders_in_memory": len(self.orders), "active_positions_in_memory": len(self.positions),
            "strategies_loaded": list(self.strategies.keys()), "environment": ENVIRONMENT,
...
def place_order_api():
    try:
        ...
        order = Order(
            product_symbol=data.get('product_symbol'), size=data.get('size'), side=data.get('side'),
            order_type=data.get('order_type', 'market_order'), limit_price=str(data.get('limit_price')) if data.get('limit_price') is not None else None,
            stop_price=str(data.get('stop_price')) if data.get('stop_price') is not None else None, client_order_id=data.get('client_order_id')
        )
...
def run_backtest():
    try:
        ...
        result = trading_system.backtest_engine.run_backtest(strategy, symbol, start_date, end_date, initial_capital)
        return jsonify(result)
    except Exception as e:
    ...
    try:
        ...
        with trading_system.db_manager.get_connection() as conn:
            cursor = conn.execute(f'''
                SELECT id, strategy_name, symbol, start_date, end_date, total_return, 
                       sharpe_ratio, max_drawdown, total_trades, win_rate, created_at 
    ...
    try:
        ...
        for name, strategy in trading_system.strategies.items():
            strategies_info[name] = {
                "name": strategy.name, "type": type(strategy).__name__,
                "config": {
...
def create_strategy():
    try:
        ...
        if not strategy_name: return jsonify({"error": "Strategy name is required"}), 400
        if strategy_name in trading_system.strategies: return jsonify({"error": f"Strategy '{strategy_name}' already exists"}), 400
...
def delete_strategy(strategy_name: str):
    try:
        if strategy_name not in trading_system.strategies: return jsonify({"error": f"Strategy '{strategy_name}' not found"}), 404
...
def health_check():
    return jsonify({
        "status": "healthy", "timestamp": datetime.now().isoformat(), "environment": ENVIRONMENT,
        ...
        "websocket_status": trading_system.ws_manager.is_connected, "system_running": trading_system.is_running
    })
...
def cleanup_database():
    try:
        ...
        market_data_deleted, trades_deleted = 0, 0
        with trading_system.db_manager.get_connection() as conn:
            cursor = conn.execute('DELETE FROM market_data WHERE timestamp < ?', (cutoff_timestamp,))
            market_data_deleted = cursor.rowcount
            cursor = conn.execute('DELETE FROM trades WHERE timestamp < ?', (cutoff_timestamp,))
        ...
        return jsonify({"message": "Database cleanup completed", "market_data_deleted": market_data_deleted, "trades_deleted": trades_deleted, "cutoff_date": cutoff_datetime.isoformat()})
    except Exception as e:
